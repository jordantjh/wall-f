...
def actions():
    ...
    if state == States.IDLE:
        if action == 'start':
            ...
            abort(
                500, 'Cannot execute command "%s" in state IDLE. Beep boop beep.' % action)
        ...
        if action == 'place':
            ...
            abort(
                500, 'Cannot execute command "%s" in state PICKING. Beep boop beep.' % action)
        ...
        if action == 'done':
            ...
            abort(
                500, 'Cannot execute command "%s" in state PLACING. Beep boop beep.' % action)
        ...
        if action == 'reset':
            ...
            abort(
                500, 'Cannot execute command "%s" in state REPAIR. Beep boop beep.' % action)
        ...
        if action == 'reset':
            ...
            abort(
                500, 'Cannot execute command "%s" in state FAILED. Beep boop beep.' % action)
